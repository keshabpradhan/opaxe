...
if(!class_exists('ihcStripe')){
	class ihcStripe{
		...
		jQuery.ajax({
            url: "/wp-content/themes/fount/intel/lib/all.php?action=validate_Users_email",
            type: "post",
                data: { user_login: user_login, user_email : user_email} ,

            success: function (response) {

            	var user_response=0;
            	var result=(JSON.parse(response).reports);
            	if(result=="New User"){
            		user_response=1;
            	}
				if(password==confirm_password && password.length > 5  && confirm_password.length > 5 && chkds.checked 
					&& first_name.length >0 && last_name.length >0 && user_response==1){
					iump_stripe.open({
